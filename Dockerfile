...
# Install deps first (cached layer) using only the manifest + README
# (README is referenced by pyproject's metadata).
...
ARG EXTRAS=""
RUN uv sync ${EXTRAS} --no-dev --no-install-project
